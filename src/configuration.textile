...
In order to modify the default configuration, you must first register the @blueCollar@ section in your application's configuration file. You can do this by adding a new @<section/>@ element under @configuration@ <i class="icon-chevron-right"></i> @configSections@:
...
If you only care about IIS7, you can leave the default Blue Collar @handlerUrl@ value alone, and add a new item to the @configuration@ <i class="icon-chevron-right"></i> @system.webServer@ <i class="icon-chevron-right"></i> @handlers@ collection:
