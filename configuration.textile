...
Blue Collar is configured using the standard .NET configuration system. Just like any other custom configuration section, you first need to register it in your application's @.config@ file (usually @Web.config@ or @App.config@). Once registered, you can update the configurable properties to suit your needs.
...
The following attributes are available on the root @<blueCollar/>@ section element, and control global behavior:
...
The following attributes are availble on the @blueCollar/dashboard@ element, and control web management dashboard behavior:
...
The following attributes are available on the @blueCollar/machine@ element, and control machine behavior:
...
The following attributes are available on the @blueCollar/repository@ element, and control repository behavior:
...
There are a number of ways to register HTTP handlers in ASP.NET. We will go over two here: IIS7+ only (the simplest), and "universal" (IIS 5.5+, plus the Visual Studio built-in web server Cassini). Feel free to mix and match to suit your application's needs.
...
Using the above configuration, you can visit the web management dashboard by navigating to @~/collar@.
...
Using the above configuration, you can visit the web management dashboard by navigating to @~/collar.ashx@.
